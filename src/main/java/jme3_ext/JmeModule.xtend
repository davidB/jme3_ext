...
@Module() class JmeModule {
    @Provides def Application application(SimpleApplication app) {
        return app
    }

    @Provides def AssetManager assetManager(SimpleApplication app) {
        return app.getAssetManager()
    }

    @Provides def AppStateManager stateManager(SimpleApplication app) {
        return app.getStateManager()
    }

    @Provides def InputManager inputManager(SimpleApplication app) {
        return app.getInputManager()
    }

    // -- Jfx-jme
    @Provides @Singleton def ICursorDisplayProvider cursorDisplayProvider(SimpleApplication app) {
